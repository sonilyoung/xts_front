...
export const EduGroup_M = ({ ...props }) => {
    ...
    // 중분류 API 호출
    const handleEduGroup = async (procGroupCd) => {
        ...
        setdataSource([
            ...eduGroupresponse?.data?.RET_DATA.map((m, i) => ({
                key: m.procGroupCd,
                rowdata0: i + 1,
        ...
        if (dataEdit === true) {
            Modal.success({
                ...
                onOk() {
                    setOpen(false);
                    setDataEdit(false);
                    handleEduGroup(props.procGroupCd_M);
                    form.resetFields();
                }
            });
        } else {
    ...
    const handleEduGroup_S = (procGroupCd_S) => {
        props.EduGroup_Call_S(procGroupCd_S);
    };
